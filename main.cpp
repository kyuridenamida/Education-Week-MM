...
struct WeightedRange {
	int left,right;
	int weight;
	WeightedRange(int left,int right,int weight) : left(left), right(right), weight(weight) {}
	bool operator < (const WeightedRange &op) const{
...
public:
	...
	vector<int> output(){
		...
		for(int i = 0 ; i < N ; i++) height[i] = make_pair(perm[i], i);
